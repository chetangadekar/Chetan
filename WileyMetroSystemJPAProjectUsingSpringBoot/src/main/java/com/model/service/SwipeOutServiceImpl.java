...
public class SwipeOutServiceImpl implements SwipeOutService {
	...
	@Override
	public boolean swipeOutPassenger(int cardNumber, int stationId) throws Exception {
//		int transactionId = swipeOutDao.getTransactionId(cardNumber);
		...
		int fare=Math.abs(stationId-swipeInId)*5;
		int fine=0;
		int rows=swipeOutDao.UpdateTransaction(stationId, time, date, fare, fine,transactionId);
		if(rows>0) {
