...
public class RegisterController {
	...
	@RequestMapping("/swipeIn")
		...
		String message=null;
		if(swipeInService.swipeInPassenger(card.getCardNumber(), station.getStationId())) {
	...
	public ModelAndView swipeOut(@ModelAttribute("command")Station station,Model model) throws Exception {
		...
		if(balance>25) {
		if(swipeOutService.swipeOutPassenger(card.getCardNumber(),station.getStationId())) {
		message="Swiped out Succesfully your fare is : "+swipeOutService.getFare(card.getCardNumber());
		return new ModelAndView("afterOutput","message",message);
		}else {
	...
	{	
		CardDetails card=(CardDetails)model.getAttribute("card");
		if(registerService.updateBalance(card.getCardNumber(), balance)) {
